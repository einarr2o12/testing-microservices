...
curl -X POST http://157.230.196.214:8001/services/product-service/routes \
  --data "name=product-service" \
...
curl -X POST http://157.230.196.214:8001/services/review-service/routes \
  --data "name=review-service" \
...
curl -X POST http://157.230.196.214:8001/services/frontend-service/routes \
  --data "name=frontend-service" \
  --data "paths[]=/" \
  --data "strip_path=true"
