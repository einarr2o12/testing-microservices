...
KONG_ADMIN_URL="http://167.172.7.95:8001"

# Create the Category Service
curl -X POST http://167.172.7.95:8001/services \
...
curl -X POST http://167.172.7.95:8001/services/category-service/routes \
  --data "name=category-service" \
  --data "paths[]=/category"
...
curl -X POST http://167.172.7.95:8001/services/product-service/routes \
  --data "name=product-service" \
  --data "paths[]=/product"
...
curl -X POST http://167.172.7.95:8001/services/review-service/routes \
  --data "name=review-service" \
  --data "paths[]=/review"
...
# Create the Review Route
curl -X POST http://167.172.7.95:8001/services/frontend-service/routes \
  --data "name=frontend-service" \
  --data "paths[]=/"
